...
#ifdef PELELM_USE_MF
void
PeleLM::fillPatchMF(const TimeStamp &a_time)
{
  BL_PROFILE("PeleLM::fillPatchMF()");
  for (int lev = 0; lev <= finest_level; lev++) {
    auto ldata_p = getLevelDataPtr(lev, a_time);
    Real time = getTime(lev, a_time);
  ...
  int nGhost)
{
  ...
  if (lev == 0) {
    ...
    PhysBCFunct<GpuBndryFuncFab<PeleLMCCFillExtDirMF>> bndry_func(
      geom[lev], fetchBCRecArray(FIRSTMFVAR, NUMMFVAR),
      PeleLMCCFillExtDirMF{lprobparm, lpmfdata, m_nAux});
    FillPatchSingleLevel(
      a_mf, IntVect(nGhost), a_time,
      {&(m_leveldata_old[lev]->state),&(m_leveldata_new[lev]->state)},
      {m_t_old[lev], m_t_new[lev]}, FIRSTMFVAR, mf_comp, NUMMFVAR, geom[lev], bndry_func,
      0);
  } else {
    ...
    PhysBCFunct<GpuBndryFuncFab<PeleLMCCFillExtDirMF>> crse_bndry_func(
      geom[lev-1], fetchBCRecArray(FIRSTMFVAR, NUMMFVAR),
      PeleLMCCFillExtDirMF{lprobparm, lpmfdata, m_nAux});
    PhysBCFunct<GpuBndryFuncFab<PeleLMCCFillExtDirMF>> fine_bndry_func(
      geom[lev], fetchBCRecArray(FIRSTMFVAR, NUMMFVAR),
      PeleLMCCFillExtDirMF{lprobparm, lpmfdata, m_nAux});
    FillPatchTwoLevels(
      a_mf, IntVect(nGhost), a_time,
      ...
      crse_bndry_func, 0, fine_bndry_func, 0, refRatio(lev-1),
      mapper, fetchBCRecArray(FIRSTMFVAR, NUMMFVAR), 0);
  }
}

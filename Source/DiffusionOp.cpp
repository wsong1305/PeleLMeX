                                          ...
                                          Real a_dt)
{
   ...
   AMREX_ASSERT( (m_pelelm->m_solver==NSSolver::LowMachNumber && have_density) ||
                 (m_pelelm->m_solver==NSSolver::Incompressible && !have_density) );
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
       ...
       for (MFIter mfi(rhs[lev],TilingIfNotGPU()); mfi.isValid(); ++mfi) {
           ...
           amrex::ParallelFor(bx, AMREX_SPACEDIM, [=,rho_incomp=m_pelelm->m_rho,
                                                     is_incomp=m_pelelm->m_solver==NSSolver::Incompressible]
           AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
           {
               if (is_incomp) {

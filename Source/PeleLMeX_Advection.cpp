...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(advData->Forcing[lev], TilingIfNotGPU()); mfi.isValid();
      ...
      amrex::ParallelFor(
        bx,
        [rho, rhoY, T, rhoMF, dn, ddn, r, fY, fT, fMF, extRhoY, extRhoH, dp0dt=m_dp0dt,
         is_closed_ch=m_closed_chamber,
         do_react=m_do_react] AMREX_GPU_DEVICE (int i, int j, int k) noexcept {
           buildAdvectionForcing(
             i, j, k, rho, rhoY, T, rhoMF, dn, ddn, r, extRhoY, extRhoH, dp0dt,
             is_closed_ch, do_react, fY, fT, fMF );
      });
#else
      amrex::ParallelFor(
        bx,
  ...
  for (int lev = 0; lev <= finest_level; ++lev) {
    for (int idim = 0; idim < AMREX_SPACEDIM; idim++) {
      fluxes[lev][idim].define(
        amrex::convert(grids[lev], IntVect::TheDimensionVector(idim)),
#ifdef PELELM_USE_MF
        dmap[lev], NUM_SPECIES + 1 + NUMMFVAR, 0, MFInfo(), 
        Factory(lev)); //Species + RhoH + MF
#else
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(ldata_p->state,TilingIfNotGPU()); mfi.isValid(); ++mfi) {
      ...
      HydroUtils::ComputeFluxesOnBoxFromState(
        bx, NUMMFVAR, mfi, rhoY_arr, AMREX_D_DECL(fx,fy,fz),
        AMREX_D_DECL(edgex,edgey,edgez), knownEdgeState,
        AMREX_D_DECL(umac, vmac, wmac), divu_arr, force_arr, geom[lev], m_dt,
...
#ifdef PELELM_USE_MF
     advFluxDivergence(
       lev, advData->AofS[lev], FIRSTMFVAR, divu, GetArrOfConstPtrs(fluxes[lev]),
       NUM_SPECIES + 1, GetArrOfConstPtrs(fluxes[lev]),
       NUM_SPECIES + 1, // This will not be used since none of rhoY/rhoH in convective
       NUMMFVAR, AdvTypeMF_d.dataPtr(), geom[lev], -1.0,
       fluxes_are_area_weighted);
#endif
#endif
  }

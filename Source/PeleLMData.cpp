...
#ifdef PELELM_USE_MF
	diff_cc.define (ba, dm, NUM_SPECIES+3 , 1           , MFInfo(), factory);
#else
                                         ...
                                         int is_init)
{
   if (is_init) {                   // All I need is a container for a single diffusion term
...
#ifdef PELELM_USE_MF	
         Dnp1[lev].define(ba[lev], dm[lev], NUM_SPECIES+3 , nGrowAdv, MFInfo(), *factory[lev]);
#else
      ...
      for (int lev = 0; lev <= a_finestLevel; lev++ ) {
#ifdef PELELM_USE_MF
	 Dn[lev].define(ba[lev], dm[lev], NUM_SPECIES+3 , nGrowAdv, MFInfo(), *factory[lev]);
	 Dnp1[lev].define(ba[lev], dm[lev], NUM_SPECIES+3 , nGrowAdv, MFInfo(), *factory[lev]);
	 Dhat[lev].define(ba[lev], dm[lev], NUM_SPECIES+3 , nGrowAdv, MFInfo(), *factory[lev]);
#else
...
#else
#ifdef PELELM_USE_MF
         Forcing[lev].define(ba[lev], dm[lev], NUM_SPECIES+2, nGrowAdv, MFInfo(), *factory[lev]); // Species + TEMP + MF
#else
...
PeleLM::copyTransportOldToNew() {
   for (int lev = 0; lev <= finest_level; lev++ ) {
      ...
      if ( !m_incompressible ) {
#ifdef PELELM_USE_MF         
         MultiFab::Copy(m_leveldata_new[lev]->diff_cc,m_leveldata_old[lev]->diff_cc,0,0,NUM_SPECIES+3,1);
#else
...
PeleLM::copyDiffusionOldToNew(std::unique_ptr<AdvanceDiffData> &diffData) {
   for (int lev = 0; lev <= finest_level; lev++ ) {
#ifdef PELELM_USE_MF     
      MultiFab::Copy(diffData->Dnp1[lev],diffData->Dn[lev],0,0,NUM_SPECIES+3,m_nGrowAdv);
#else

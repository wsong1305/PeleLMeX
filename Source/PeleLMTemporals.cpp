...
void PeleLM::initTemporals(const PeleLM::TimeStamp &a_time)
{
   ...
   if (m_do_massBalance && m_solver==NSSolver::LowMachNumber) {
   ...
   if (m_do_energyBalance && m_solver==NSSolver::LowMachNumber) {
   ...
   if (m_do_speciesBalance && m_solver==NSSolver::LowMachNumber) {
...
void PeleLM::writeTemporals()
{
   ...
   auto stateMax  = ( m_solver==NSSolver::LowMachNumber ) ? MLmax(GetVecOfConstPtrs(getStateVect(AmrNewTime)),0,NVAR)
                                                              : MLmax(GetVecOfConstPtrs(getStateVect(AmrNewTime)),0,AMREX_SPACEDIM);
   auto stateMin  = ( m_solver==NSSolver::LowMachNumber ) ? MLmin(GetVecOfConstPtrs(getStateVect(AmrNewTime)),0,NVAR)

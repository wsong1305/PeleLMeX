  ...
  ProbParm const& prob_parm)
{
  ...
  jet_norm[AMREX_SPACEDIM - 1] = 1.;
  int numProcInj = 1; // BRUNO: you can play around with this if the injection needs more procs
  int startProcInj = 0;
  if (prob_parm.part_stdev_dia >= 0.) {
    ...
    sprayInjection<LogNormDist>(
      ...
      prob_parm.Y_jet.data(), lev, startProcInj, numProcInj, true);
  } else {
    WeibullDist w_dist(prob_parm.part_mean_dia, prob_parm.part_weibull_k);
    sprayInjection<WeibullDist>(
      w_dist, prob_parm.jet_cent, jet_norm, prob_parm.jet_dia, prob_parm.part_temp,
      prob_parm.mass_flow_rate, jet_vel, prob_parm.spread_angle, dt,
      prob_parm.Y_jet.data(), lev, startProcInj, numProcInj, true);
  }

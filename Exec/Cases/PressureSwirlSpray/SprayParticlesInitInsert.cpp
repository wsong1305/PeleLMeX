...
//     prob_parm.floating_injection_mass += mass_flow_rate * dt;
  ...
  */
  ...
  while (total_mass < injection_mass) {
      ...
      // Get the random position on the azimuth
      ...
      RealVect part_loc(AMREX_D_DECL(cur_jet_cent[0],
                                     ...
                                     cur_jet_cent[2] + std::cos(inj_p_angle_random) * inj_p_rad));
      ...
      AMREX_D_TERM(Real x_vel = jet_vel;,
                   Real y_vel = std::cos(inj_p_angle_random) * ut_vel;,
                   Real z_vel = -std::sin(inj_p_angle_random) * ut_vel);
